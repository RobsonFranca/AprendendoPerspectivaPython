...
class Visao():
    def __init__(self, x, y, altura = 0):
        ...
        '''
        #linha horizontal
        self.p1 = deAngulo(radians(-30),55)
        self.p2 = deAngulo(radians(30),55)
        self.linha_horizontal = Linha(self.p1.x,self.p1.y,
                                      self.p2.x,self.p2.y)
        #linha vertical
        self.p3 = deAngulo(radians(self.angulo_vertical-30),55)
        self.p4 = deAngulo(radians(self.angulo_vertical+30),55)
    ...
    def rodar(self, n):
        ...
        self.p1 = deAngulo(radians(-30+self.angulo_horizontal),55)
        self.p2 = deAngulo(radians(30+self.angulo_horizontal),55)
        self.linha_horizontal = Linha(self.p1.x,self.p1.y,
                                      self.p2.x,self.p2.y)
    ...
    def renderizar(self, objeto, tela):
        ...
        for p in objeto.pontos:
            ...
            if ponto != None:
                ...
                posicao_ponto = Ponto(distancia,p.z)
        ...
        if len(pontos) > 2:
            return {'pontos':pontos,'cor':objeto.cor,'distancia':r_distancia/len(pontos)}
